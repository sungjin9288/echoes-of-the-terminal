...
### 추가 (Added)
- **Pack 29 — 심해 법정 (Abyssal Court)** (`packs/pack_29_abyssal_court.json`): 2170년대 심해 광물 채굴 기지를 배경으로 한 수사물 팩.
  - node_id 1029-1033, 테마 `ABYSSAL_COURT_A~E`, Easy×2 / Hard×2 / NIGHTMARE×1 분포.
  - 넵투누스-4 기지 망간단괴 절도 — 수심 4,200m에서 햇빛 목격 모순 (햇빛, Easy)
  - 심해 통신 장치 파손 사건 — 위성전화 통화 물리 불가 모순 (위성전화, Easy)
  - 심해 외벽 잠수 사기 — 420기압 환경에서 잠수복 없이 생존 불가 (잠수복, Hard)
  - 기지 분석실 방화 — 산소는 화재를 진압하지 않고 촉진 (산소, Hard)
  - 잠수정 사망 사건 — 수심 4,800m 급부상 시 감압병 즉사 (감압, NIGHTMARE)
- **Pack 29 테스트 21케이스** (`tests/test_new_packs_v14.py`): 829 → **850 케이스**.
  - 파일 존재·로드·메타데이터·시나리오 수·node_id 범위·순차성·필드 검증·난이도 분포·테마 접두어·is_boss·text_log 길이·키워드 in text_log·패널티-난이도 매핑·예상 키워드·logical_flaw_explanation·NIGHTMARE 최고 패널티·심해 배경 키워드·팩 간 충돌·전체 고유성.
