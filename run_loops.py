"""런 루프 핵심 로직 — 일반 런과 데일리 챌린지 실행 엔진.

run_game_session()과 run_daily_challenge()를 공개 API로 제공한다.
런타임 빌더, 어센션 수정자, 전투 풀 로딩 등 런 전용 헬퍼도 함께 정의한다.
"""
...
from artifact_system import Artifact
from combat_commands import calculate_analyze_penalty as _calculate_analyze_penalty_impl
...
from ui_renderer import (
    ...
    render_route_choice,
    type_text,
    wait_for_enter as _wait_for_enter,
)
...
def _mutate_route_choices_for_ascension(
    ...
    # 최소 ELITE 선택지 개수 보장
    elite_choices = sum(
        1 for left, right in mutated for node in (left, right) if node == NodeType.ELITE
    )
    forced_elite = 0
...
def run_game_session(
    ...
    Returns:
        (correct_answers, is_victory, result, node_difficulties_cleared, run_stats)
    """
    ...
    pools = _load_combat_pools(ascension_level=ascension_level)
    if pools is None:
        return 0, False, "aborted", [], {"wrong_analyzes": 0, "timeout_events": 0}
    combat_pool, boss_scenario, boss_phase_pack = pools
    ...
    for position in range(total_positions):
        ...
        if pending_bonus > 0:
            ...
            runtime["time_limit_seconds"] = int(runtime.get("time_limit_seconds", 30)) + pending_bonus
            console.print(
                f"[bold #00FFFF][PULSE BARRIER] 이번 노드 제한시간 +{pending_bonus}초 보정[/bold #00FFFF]"
            )

        # argos_fragment: 노드 시작 시 자동 추적도 감소
        ...
        if per_node_reduce > 0 and trace_level > 0:
            ...
            trace_level -= reduced
            console.print(
                f"[bold magenta][ARGOS FRAGMENT] 추적도 자동 -{reduced}%  현재: {trace_level}%[/bold magenta]"
            )
        ...
        if ntype == NodeType.REST:
            ...
            type_text(
                f"[REST NODE] 시스템 재조정 완료.\n추적도 -{heal_amount}%{bonus_str}  현재: {trace_level}%",
                style="bold cyan",
                delay=0.03,
            )
            _wait_for_enter()
            ...
            if ntype == NodeType.BOSS:
                ...
                for phase_index in range(1, total_boss_phases + 1):
                    ...
                    phase_scenario = _apply_asc20_boss_phase_override(
                        scenario, phase_runtime, boss_phase_pack,
                    )
                    ...
                    if combat_result == "death":
                        return (
                            correct_answers, False, "shutdown",
                            node_difficulties_cleared,
                            _build_run_stats(run_state, trace_level, max_trace_reached, acquired_artifacts),
                        )
    ...
    return (
        correct_answers, True, "victory",
        node_difficulties_cleared,
        _build_run_stats(run_state, trace_level, max_trace_reached, acquired_artifacts, is_victory=True),
    )
...
def run_daily_challenge(save_data: dict[str, Any]) -> None:
    ...
    console.print(f"[bold cyan][DAILY SEED] 오늘의 시드가 로드되었습니다 — {today}[/bold cyan]")
