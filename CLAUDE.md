...
│   └── pack_29_abyssal_court.json     # 팩 29 (심해 법정, 5개 시나리오, node_id 1029~1033)
...
│
├── scenarios.json           # 280개 시나리오 데이터 (Pack 01-22)
...
│
├── web/                     # FastAPI + htmx 웹 UI (v2.0)
│   ├── __init__.py
│   ├── adapters.py          # ConsoleBridge (thread-local) + WebGameSession + install_patches()
│   ├── session.py           # SessionStore (인메모리, TTL 1시간)
│   ├── app.py               # FastAPI 앱 — 라우트 14종 (+ /records, /achievements, /endings, /api/daily/start, /api/settings/theme|lang) + lifespan 정리 태스크
...
│
├── tests/                   # pytest 테스트 (34파일, 886케이스, 커버리지 81%+)
...
| `test_web_session.py` | 96 | 로비·쿠키·SSE·레이트 리밋·세션·records·데일리·테마·i18n·achievements·endings 갤러리 |
